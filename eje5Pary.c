 ...
 * P0 = 0000 0000 0000 0111 1000 1111 1111 1111
*/
#include "LPC17xx.h"
...
int main(void){
    ...
    LPC_GPIO0->FIODIR |= (0x78FFF<<0);   //Configuracion para output P0.0 al P0.11 y P0.15 al P0.18
    uint32_t value[8] = {0,0,0,0,0,0,0,0};
    uint32_t avgvalue;
    int i = -1;
    while(1){
        i++;
        LPC_GPIO0->FIOCLR |= (0x78FFF<<0);
        delay();
        value[i] += (LPC_GPIO1->FIOPIN & 0xFFFFF);
        avgvalue = average(value);
        if(i >= 7){
            i = -1;
        }
        uint32_t avg1 = avgvalue & 0x00FFF;
        uint32_t avg2 = avgvalue & 0X78000;
        LPC_GPIO0->FIOSET |= (avg1<<0);
        LPC_GPIO0->FIOSET |= (avg2<<0);
    }
